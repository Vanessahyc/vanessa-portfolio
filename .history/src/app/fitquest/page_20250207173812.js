...
export default function Fitquest() {
    ...
    const settings = {dots: true, infinite: true, speed: 500, slidesToShow: 3, slidesToScroll: 1, autoplay: false, autoplaySpeed: 3000, centerMode: true, centerPadding: '10px'};
                        ...
                        <Slider {...settings}>
                            {images.map((src, index)=> (
                                <div key={index} className="cursor-pointer">
                                    <img src={src} alt={`slide ${index+1}`} className="w-full h-auto mr-1"></img>
                                </div>
                            ))}
                        </Slider>
                        {/* <Image
